        ...
        if (getControlItem.name === "phone") {
          element = (
            <Input
              ...
              onBlur={() => {  // “User ne input chhod diya (click bahar / tab press)” → function run karo
                if (value.length !== 10) {
          ...
          element = (
            <Input
              ...
              value={value}
              onChange={(event) => {
                const newValue = event.target.value;  // User ne jo type kiya, wo value le rahe ho
                if (/^\d*$/.test(newValue) && newValue.length <= 6) {  // “Kya value me sirf digits (0–9) hi hain?” and value 6 ya 6 se kam h to ok 
                  setFormData({
                    ...formData,
                    [getControlItem.name]: newValue,
                  });
              ...
              onBlur={() => {
                if (value.length !== 6) {  // exact 6 hona chiaye 
...
//👉 Aur user jo option choose karega → wo state me save ho raha hai
      case "select":
        element = (
          <Select  // Ye dropdown component hai
            onValueChange={(value) =>  // Jab user koi option select kare:
              setFormData({
                ...formData,
                [getControlItem.name]: value,  // 👉 Field ka naam - [getControlItem.name]
              })
            }
        ...
        element = (
          <Textarea
            ...
            onChange={(event) =>
              setFormData({
                ...formData,
                [getControlItem.name]: event.target.value,  // “jo hum type kar rahe wo UI me dikhega .name se” nhi 
