...
// 🔥 Guest cart localStorage me store hoga
const GUEST_CART_KEY = "guestCart";

// 🔥 localStorage se guest cart lene ke liye helper function
...
// 🔥 Guest cart ko localStorage me save karne ke liye helper function
function saveGuestCart(cartItems) {
  localStorage.setItem(GUEST_CART_KEY, JSON.stringify(cartItems));
}

// 🔥 Guest cart ko Redux ke existing cart structure jaisa bana rahe hain
function createGuestCartData(items) {
  return {
    items,
  };
}
...
export const addToCart = createAsyncThunk( // Exports the thunk so you can dispatch it from components:
                                           //dispatch(addToCart({ userId, productId, quantity }))
  "cart/addToCart",  // redux ko btata h kaunsa async action chal raha hai  structure is "sliceName/actionName" yha slice ka name cart h aur action ka name addToCart h  kya ye apne man se likhte h YES ✅ (apne man se likhte ho) but meaningful hona chahiye taki debugging me help mile      ye use hota hai -> to identify the action in Redux DevTools and in reducers.
  async ({ userId, productId, quantity, product }) => {    // Pehle → tumne values receive ki (from dispatch) // add to cart me ye teno use kiye gai h cartcontroller.js , async function that runs when you dispatch the thunk.    Mujhe ek object milega, usme se directly ye 3 values nikaal lo userId, productId, quantity.  Ye values aapko component se milengi jab aap dispatch karoge addToCart ko.  yha pr hum async ({data}) => { bhi likh sakte the, but isse hum directly data ke andar se userId, productId, quantity nikal sakte hain. this is called object destructuring in JavaScript.

    // 🔥 Agar user logged in hai → existing backend cart use hoga
    if (userId) {
      const response = await axios.post(  // Sends a POST request to your backend at http://localhost:5000/api/shop/cart/add
        `${import.meta.env.VITE_API_URL}/api/shop/cart/add`, // ye krne se cart-route me req jaegi
        {
          userId, // Ab → unhe backend ko send kar rahe ho
          productId,
          quantity,
        }
      ); // the second argument of axios.post(url, body) is the request body.
    ...
    // 🔥 Agar user logged in nahi hai → localStorage guest cart use hoga
    ...
    return {
      success: true,
      data: createGuestCartData(guestCart),
      isGuestCart: true,
    };
  }
);
...
export const fetchCartItems = createAsyncThunk(
  ...
  async (userId) => {  // This means the function takes one argument called userId. not an object.
    ...
    // 🔥 Guest user ke liye MongoDB se cart fetch nahi karna
    if (!userId) {
      const guestCart = getGuestCart();
...
export const deleteCartItem = createAsyncThunk(
  ...
  async ({ userId, productId }) => {
    ...
    if (!userId) {
      ...
      return {
        success: true,
        data: createGuestCartData(updatedGuestCart),
        isGuestCart: true,
      };
    }
...
export const updateCartQuantity = createAsyncThunk(
  ...
  async ({ userId, productId, quantity }) => {
    ...
    // 🔥 Guest cart ki quantity localStorage me update
    if (!userId) {
      const guestCart = getGuestCart();
...
// 🔥 Guest cart ko login ke baad MongoDB cart me transfer karne ke liye
  ...
  async (userId) => {
    ...
    if (guestCart.length === 0) {
      return {
        success: true,
        data: null,
        isGuestCartSynced: false,
      };
    }

    // 🔥 Guest cart ke har product ko existing backend cart me add karenge
    ...
    // 🔥 Sync hone ke baad guest cart clear kar do
    localStorage.removeItem(GUEST_CART_KEY);

    return {
      success: true,
      isGuestCartSynced: true,
    };
  }
);
...
// “extraReducers is used to handle async thunk actions.”
  ...
  extraReducers: (builder) => {  // builder -> Ye ek helper object hai “Kaunsa action aaye to kya karna hai” define karne ke liye. Ye createAsyncThunk ke actions ko handle karne ke liye use hota hai. Jab aap createAsyncThunk se koi async action banate hain, to uske pending, fulfilled, aur rejected states ko handle karne ke liye extraReducers me builder ka use karte hain.
    builder
      .addCase(addToCart.pending, (state) => {  // .addcase -> “Jab ye action aaye → ye function chalao” define karta hai. addToCart.pending ka matlab hai “Jab addToCart action chal raha ho (pending state) to ye function chalao”. Ye function state ko update karega jab addToCart action pending state me hoga. yha pr hum isse isliye use kar rhe h taki jab bhi addtocart action chal raha ho to hum isLoading ko true kr de taki user ko pata chale ki kuch ho rha h
        state.isLoading = true;
      })
      .addCase(addToCart.fulfilled, (state, action) => {
        state.isLoading = false;
        state.cartItems = action.payload.data; // data isliye kyu ki cartcontroler.js me data pass kiya h cart response me
      })
      .addCase(addToCart.rejected, (state) => {
        state.isLoading = false;
        state.cartItems = [];
